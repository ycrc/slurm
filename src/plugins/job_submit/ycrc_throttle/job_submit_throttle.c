 ...
 */
const char plugin_name[]       	= "Job submit throttle plugin for YCRC";
const char plugin_type[]       	= "job_submit/ycrc_throttle";
		      ...
		      char **err_msg)
{
	...
	// ycrc qos for bypassing throttle
	if (job_desc->qos) {
		if (strcmp(job_desc->qos, "nothrottle") == 0) {
			debug2("job submit throttle bypassed via QOS");
			return SLURM_SUCCESS;
		}
	}
	// end ycrc qos for bypassing throttle
	...
	for (i = 0; i < thru_put_size; i++) {
		...
		if (err_msg)
			*err_msg = xstrdup("You have reached the rate limit of jobs submitted per hour. To avoid hitting this limit\n\
in the future, please use job arrays to submit large numbers of jobs instead.\n\
For more information: https://docs.ycrc.yale.edu/clusters-at-yale/job-scheduling/common-job-failures/#rate-limits");
		slurm_mutex_unlock(&throttle_mutex);
